...
namespace BFS
{
    internal class BreathFirstSearch
    {
        ...
        public bool Solve()
        {
            ...
            currentState += _startP.ToString();
            currentState += string.Concat(Enumerable.Repeat(' ', numOfDash - 2));
            currentState += "|";

            string listNeighbors = _nodeList[mapVertices[_startP]].ListNeighborsToString();
            currentState += listNeighbors;
            currentState += string.Concat(Enumerable.Repeat(' ', numOfDash - listNeighbors.Length));
            currentState += "|";

            bool checkFirstTime = true;

            stringToExport += currentState;
            ...
            while (q.Count > 0)
            {
                ...
                if (checkFirstTime == false)
                {
                    if (adjacentList.Length > 2)
                        adjacentList = adjacentList.Substring(0, adjacentList.Length - 2);
